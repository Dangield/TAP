clear;
close all;
clc;

addpath('./abstraction')
addpath('./classes')
addpath('./..')

consts

obj = NonlinearReactor();

workpoint = struct('u', [2; 15], 'y', [0.2646; 393.9521], 'd', [323; 365]);
obj.resetToWorkPoint(workpoint);

D = 800;
N = 50;
Nu = 50;
lambda = [0.1 0.1];
psii = 1;
sim_length = 10000;
setPoints = build_random_setpoints_array(workpoint, sim_length, 1000, [0.2 385] , [0.3 400]);

% setPoints(1, 1000:end) = 0.27;

% setPoints(2, 1000:end) = 0.27;

load('./data/s.mat', 's');
reg = DMC_Regulator(obj, workpoint, s, D, N, Nu, lambda, psii);

u = workpoint.u.*ones(obj.nu, sim_length);
y = workpoint.y.*ones(obj.ny, sim_length);

for k = 1:sim_length
    output = obj.getOutput();
    y(:, k) = output;
    control = reg.calculate(output, setPoints(:, k));
    u(:, k) = control';
    obj.setControl(control);
    obj.nextIteration();
end

figure;
for i = 1:2
    subplot(2, 1, i);
        hold on;
        stairs(setPoints(i, :), 'b');
		stairs(y(i, :), 'r');
end

% figure;
% for i = 1:2
%     subplot(2, 1, i);
% 		plot(u(i, :), 'r');
% end